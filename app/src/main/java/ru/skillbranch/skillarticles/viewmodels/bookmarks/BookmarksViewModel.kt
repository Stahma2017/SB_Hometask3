...
class BookmarksViewModel(handle: SavedStateHandle) : BaseViewModel<BookmarksState>(handle, BookmarksState()) {
    ...
    private val listData = Transformations.switchMap(state) {
        when {
            ...
            else -> buildPagedList(repository.bookmarkedArticles())
        }
    }
    ...
    private fun buildPagedList(dataFactory: ArticlesDataFactory) : LiveData<PagedList<ArticleItemData>> {
        ...
        if (dataFactory.strategy is ArticleStrategy.BookmarkArticles) {
            builder.setBoundaryCallback(BookmarksBoundaryCallback(::zeroLoadingHandle, ::itemAtEndHandle))
        }
    ...
    private fun zeroLoadingHandle() {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            val items = repository.findBookmarkedArticles(start = 0, size = listConfig.initialLoadSizeHint)
            Log.d("TEST18", "zeroLoadingHandle  repository.findBookmarkedArticles(0, ${listConfig.initialLoadSizeHint})")
            if (items.isNotEmpty()) {
    ...
    private fun itemAtEndHandle(lastLoadArticle: ArticleItemData) {
        viewModelScope.launch(Dispatchers.IO) {
            ...
            withContext(Dispatchers.Main) {
                notify(Notify.TextMessage("Load bookmarks from ${items.firstOrNull()?.id} " + "to ${items.lastOrNull()?.id}"))
            }
        }
    }
