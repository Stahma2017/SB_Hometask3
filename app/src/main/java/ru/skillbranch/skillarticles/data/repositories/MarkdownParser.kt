...
object MarkdownParser {
    ...
    const val MARKDOWN_GROUPS = "$UNORDERED_LIST_ITEM_GROUP|$HEADER_GROUP|$QUOTE_GROUP|$ITALIC_GROUP" +
            "|$BOLD_GROUP|$STRIKE_GROUP|$RULE_GROUP|$INLINE_GROUP|$LINK_GROUP|$ORDER_LIST_GROUP|$BLOCK_CODE_GROUP|$IMAGE_GROUP"
    ...
    fun findElements(string: CharSequence): List<Element> {
        ...
        loop@ while (matcher.find(lastStartIndex)) {
            ...
            when (group) {
                ...
                2 -> {
                    var reg = "^#{1,6}".toRegex().find(string.subSequence(startIndex, endIndex))
                ...
                //NUMERIC LIST
                10 -> {
                    var reg = "^\\d+.".toRegex().find(string.subSequence(startIndex, endIndex))
                    val order = reg!!.value
...
sealed class MarkdownElement() {
    ...
    data class Text(
        ...
        override val offset : Int = 0) : MarkdownElement()
